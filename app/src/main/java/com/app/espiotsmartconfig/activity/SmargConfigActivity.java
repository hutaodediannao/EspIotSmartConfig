...
public class SmargConfigActivity extends AbsBaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        EspApp.getInstance().observeBroadcast(this, broadcast -> {
    ...
    private void onWifiChanged() {
        ...
        if (stateResult.wifiConnected) {
            ...
            if (mTask != null) {
                mTask.cancelEsptouch();
                mTask = null;
                new AlertDialog.Builder(SmargConfigActivity.this)
    ...
    private void executeEsptouch() {
        byte[] ssid = mSsidBytes == null ? ByteUtil.getBytesByString(this.mSsid) : mSsidBytes;
        ...
        KeyboardUtil.hideKeyboard(mBinding.confirmBtn);
    }

    private static class EsptouchAsyncTask4 extends AsyncTask<byte[], IEsptouchResult, List<IEsptouchResult>> {
        private final WeakReference<SmargConfigActivity> mActivity;

        private final Object mLock = new Object();
        private AlertDialog mResultDialog;
        private IEsptouchTask mEsptouchTask;

        EsptouchAsyncTask4(SmargConfigActivity activity) {
            mActivity = new WeakReference<>(activity);
        }

        void cancelEsptouch() {
            cancel(true);
            SmargConfigActivity activity = mActivity.get();
            if (activity != null) {
